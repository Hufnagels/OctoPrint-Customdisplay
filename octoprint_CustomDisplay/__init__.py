...
class CustomdisplayPlugin(octoprint.plugin.StartupPlugin,
	...
	def on_event(self, event, payload):
		...
		# FileSelected
		if event in (
			Events.PRINT_STARTED, Events.PRINT_DONE, Events.PRINT_PAUSED, Events.PRINT_RESUMED, Events.PRINT_CANCELLED,
			Events.PRINT_FAILED):
		...
		try:
			...
			for tempsensors in self.tempsensors['DHT11']:
				...
				try:
					...
					self.sensordata.append({'name': tempsensors['name'], 'temp': 'T:' + str(temperature_c),
											'hum': ' H:' + str(humidity) + '%'})
					time.sleep(.5)
